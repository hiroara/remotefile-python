...
def parse_args():
    ...
    usage = 'Usage: {} script [arguments ...] [--verbose] [--server] [--port <number>] [--help]'.format(os.path.basename(__file__))
...
def main():
    ...
    if args.server:
        ...
        Server(args.script, cache_dir='/tmp/src', port=args.port).serve_forever(*args.arguments)
    else:
        from remotefile.runner import Runner
        Runner(args.script, cache_dir='/tmp/src').exec_script(*args.arguments)
